...
<?php
   //Sort - ORDER BY
    ...
    $ask1="SELECT dane.nr_zawodnika, dane.imie, dane.nazwisko, dane.klasyfikacja, dane.kategoria, dane.wiek, dane.miasto,  dane.nr_chip $dodCol1 $dodCol2 $dodCol3 $dodCol4 $dodCol5 $dodCol6 $dodCol7 $dodCol8 $dodCol9 $dodCol10 $dodCol11 from dane WHERE nr_zawodnika!=0 $whereCat $whereClas $whereClub $whereCountry $whereSex $numberRuner1  ORDER BY $firstOrder $firstBy , $secondOrder $secondBy";
    if($resultat1 = mysqli_query($conn1,$ask1)){
        if(mysqli_num_rows($resultat1)){
            echo("<table><tr><th>Edytuj</th><th>Lp.</th>");
            echo("<th>".$thTables[1][0]."</th>");
            echo("<th>".$thTables[5][0]."</th>");
            echo("<th>".$thTables[6][0]."</th>");
            echo("<th>".$thTables[3][0]."</th>");
            echo("<th>".$thTables[4][0]."</th>");
            echo("<th>".$thTables[8][0]."</th>");
            echo("<th>".$thTables[11][0]."</th>");
          

           $counter3=29;
           while($counter3<count($thTables)){
                echo("<th>".$thTables[$counter3][0]."</th>");
                $counter3++;
            }
            echo("</tr>");
           $counter2=1;
